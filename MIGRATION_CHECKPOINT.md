...
- 2026-07-10 (Phase 6, QR/scanner done): Found the training-attendance scanner path was dead code (controller supported it, Vue page never exposed it) and fixed it. Rebuilt QR normalization to handle OEP payloads and legacy pipe-suffixed codes without crashing. Added full OEP scanning (identity lookup + venue-scoped attendance, FO-scoped, idempotent). 8 new tests, 5 existing scanner tests still green (no regressions). 160 tests / 956 assertions total; build clean.
...
- 2026-07-10 (Phase 7, D3 done): Built the full Other Examination Personnel module — registry CRUD with photo/ID-card/QR (mirrors Member module patterns), venue assignment, and a manual attendance toggle integrated into the examination venue cards. 14 new tests, all green; 135 tests / 807 assertions total; build clean.
...
- 2026-07-10 (later): Owner decided D1–D5 (single guard / venues yes / OEP yes / all 17 roles / confirmation workflow yes). Implemented Phase 1 remainder: 15 new migrations (users auth columns, exam_types, schools, examination_school, exam_rooms, assignment venue+confirmation columns, assignment_confirmations, OEP ×3, settings, letterheads, email_templates, email_logs, notifications) — `php artisan migrate` clean. Phase 2: ExamRole expanded to 17 with ExamRoleGroup; new enums AssignmentStatus/ConfirmationAction/PersonnelType; 12 new models (School, ExaminationSchool, ExamRoom, AssignmentConfirmation, OtherExaminationPersonnel, OepAssignment, OepAttendance, ExamType, Setting, Letterhead, EmailTemplate, EmailLog); Examination/ExamAssignment/Member/User updated (venues, status workflow, serviceHistory(), auth columns). Verified: DB smoke test (full exam→venue→room→assignment→confirmation graph + OEP + settings + letterhead + template render) passes; existing suite 43 tests / 437 assertions green.
...
- [x] **D3. OEP module → yes.** Implemented: `other_examination_personnel`, `oep_assignments`, `oep_attendances` tables + models/enums. UI in Phase 7.
...
- [x] OEP tables: `other_examination_personnel`, `oep_assignments`, `oep_attendances` (all FK-constrained — fixes legacy's missing FKs)
...
- [x] Models: School, ExaminationSchool, ExamRoom, AssignmentConfirmation (append-only), OtherExaminationPersonnel, OepAssignment, OepAttendance, ExamType, Setting (cached get/set), Letterhead (activate()), EmailTemplate ({placeholder} render()), EmailLog
...
- [x] Factories (2026-07-11): 11 of the 12 Phase-2 models now have factories, added incrementally as each feature needing them was built (School/ExaminationSchool/ExamRoom/OtherExaminationPersonnel/OepAssignment/ExamType/Letterhead from earlier phases; `AssignmentConfirmationFactory`, `OepAttendanceFactory`, `EmailTemplateFactory`, `EmailLogFactory` added this session to close the gap). `Setting` intentionally has none — it's a key-value model with no `HasFactory` trait by design; tests correctly use its own `Setting::set()`/`get()` helpers instead of a factory. Relationship feature tests exist throughout (every module's test suite exercises its relationships via real HTTP requests, not isolated unit tests) — no separate relationship-test file was ever needed.
...
- [x] OEP registry (36) + assignments (34) + attendances (13) via exam|school→venue pair map
...
Completed 2026-07-10 — QR/scanner: OEP support + legacy payload compatibility:
...
- [x] `ScannerController::normalize()` rewritten to a typed resolver: strips `/verify/{id}` URLs (unchanged), recognizes `OEP:{id}` payloads (case-insensitive) as other examination personnel, strips legacy `|attendance`-style suffixes still on old printed QR stock before falling back to a member-code lookup, and never throws on unresolvable legacy formats (e.g. bare `'7|attendance'`) — reports "not found" instead of erroring, per the audit's compatibility goal
- [x] OEP scanning added end-to-end: scanning a `OEP:` code resolves the person (FO-scoped like members); when an Examination + Venue are selected, a scan calls the same present/idempotent attendance logic as the manual toggle (`confirmOepAttendance`, mirrors `OepAssignmentController::markAttendance`) and records `scan_method: 'qr'`; without a venue selected it prompts for one instead of silently doing nothing
- [x] `Scanner/Index.vue`: added the venue selector (populated from the selected examination's venues), a distinct OEP result panel, and an Examination/Training mode toggle
- [x] Tests: `ScannerOepAndCompatibilityTest` (8 — OEP resolves, attendance recorded once, venue-required prompt, not-assigned-to-venue, FO-scope rejection, legacy pipe-suffix stripped and resolved, bare numeric legacy code doesn't crash, lowercase `oep:` prefix recognized); existing `ScannerTest` (5) still green — no regressions
...
Completed 2026-07-10 — Other Examination Personnel module (D3):
- [x] `OtherExaminationPersonnel` model: auto-generated `oep_id` (booted hook, `OEP-CSCRO8-` + 6-char unambiguous code, mirrors Member's generator) + `name` accessor
- [x] `OtherExaminationPersonnelPolicy` (view: any staff or FO-scoped own-office; manage: super_admin/esd_admin any, fo_admin own office) + Store/UpdateOtherExaminationPersonnelRequest (all fields optional except name/sex/type — matches legacy's sparse OEP data)
- [x] `OtherExaminationPersonnelController` full resource (index w/ search+type+FO filters+pagination, create, store, show, edit, update, destroy, photo) — mirrors MemberController exactly
- [x] `OepIdCard` support class + `OepIdCard.vue` component (own "Other Examination Personnel" branding distinct from the member card; QR value `OEP:{oep_id}` for scanner parsing, not a public verify URL since OEP has no self-service portal)
- [x] Pages: `OtherExaminationPersonnel/Index.vue`, `Create.vue`, `Edit.vue`, `Show.vue` (with ID card + print), `Partials/OtherExaminationPersonnelForm.vue`
- [x] `OepAssignmentPolicy` + `OepAssignmentController` (attach/detach OEP to an examination venue, unique-per-venue enforced, FO-scoped; `markAttendance` toggles a `OepAttendance` row, idempotent)
- [x] `ExaminationController::show` extended with `oep_assignments` per venue (name, id, present flag) + `availableOep`; `Examinations/Show.vue` venue cards gained an inline OEP roster with add/remove and a click-to-toggle attendance chip
- [x] Nav links added (fo_admin/esd_admin/super_admin)
- [x] Tests: `OtherExaminationPersonnelTest` (7 — CRUD, FO-scoping both directions, photo upload/retrieval, ID card data, soft delete) + `OepAssignmentTest` (7 — assign/duplicate-reject/FO-scope-reject/remove/attendance-toggle-both-ways/no-duplicate-on-repeat/examination-page-integration)
...
- [x] OEP attendance is currently manual-toggle only; QR-scan capture for OEP is already covered by Phase 6's QR-compatibility item — not a gap
