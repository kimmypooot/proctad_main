...
#[Fillable([
    'nep_id', 'first_name', 'middle_name', 'last_name', 'suffix', 'sex',
    'contact_number', 'email', 'agency', 'position', 'personnel_type',
    'field_office_id', 'photo_path', 'is_active', 'created_by',
])]
class NonExamPersonnel extends Model
{
    use Auditable, HasFactory, SoftDeletes;

    protected $table = 'non_exam_personnel';

    public const ID_PREFIX = 'NEP-CSCRO8-';
    ...
    {
        static::creating(function (self $nep) {
            $nep->nep_id ??= self::generateNepId();
        });
    }

    public static function generateNepId(): string
    {
        do {
            $code = '';
            for ($i = 0; $i < 6; $i++) {
        ...
        } while (self::withTrashed()->where('nep_id', $id)->exists());
    ...
    /**
     * NEP records are recorded in uppercase throughout (ID cards, rosters),
    ...
    {
        return $this->hasMany(NepAttendance::class);
    }
