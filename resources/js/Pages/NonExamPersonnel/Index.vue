...
<template>
    <Head title="Non-Exam Personnel" />

    <DashboardLayout>
        <DashboardPageHeader
            title="Non-Exam Personnel"
        ...
        >
            <template v-if="can.create" #actions>
                <BaseButton href="/non-exam-personnel/create" variant="primary" size="sm">
        ...
        <div class="mt-6 grid gap-4 rounded-xl border border-slate-200 bg-white p-4 sm:grid-cols-2 lg:grid-cols-3">
            <TextInput v-model="search" label="Search" placeholder="NEP ID, name, or agency" />
            <SelectInput
        ...
        <div v-if="loading || personnel.data.length" class="mt-6 overflow-x-auto rounded-xl border border-slate-200 bg-white">
            <table class="min-w-full divide-y divide-slate-200 text-sm">
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr v-for="nep in loading ? [] : personnel.data" :key="nep.id" class="transition-colors hover:bg-brand-50/40">
                        <td class="whitespace-nowrap px-3 py-2 font-mono text-xs font-semibold text-brand-700">
                            <Link :href="`/non-exam-personnel/${nep.id}`" class="hover:underline">{{ nep.nep_id }}</Link>
                        </td>
                        <td class="px-3 py-2 font-medium text-slate-900">
                            <Link :href="`/non-exam-personnel/${nep.id}`" class="hover:underline">{{ nep.name }}</Link>
                            <p class="text-xs font-normal text-slate-400 sm:hidden">{{ nep.personnel_type_label }}</p>
                        </td>
                        <td class="hidden whitespace-nowrap px-3 py-2 text-slate-600 sm:table-cell">{{ nep.personnel_type_label }}</td>
                        <td class="hidden whitespace-nowrap px-3 py-2 text-slate-600 md:table-cell">{{ nep.field_office?.name ?? 'Region-wide' }}</td>
                        <td class="px-3 py-2">
        ...
        <div v-else class="mt-6">
            <EmptyState
                ...
                description="No non-exam personnel match your search or filters."
            />
        </div>
